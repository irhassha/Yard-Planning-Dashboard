...
const DEFAULT_CAPACITY = {
    "A01": { slots: 37, tier: 5, cap: 1110 }, "A02": { slots: 37, tier: 5, cap: 1110 }, 
    "A03": { slots: 37, tier: 5, cap: 1110 }, "A04": { slots: 37, tier: 5, cap: 1110 }, 
    "A05": { slots: 37, tier: 5, cap: 1110 }, "A06": { slots: 37, tier: 5, cap: 1110 }, 
    "A07": { slots: 37, tier: 5, cap: 1110 }, "A08": { slots: 37, tier: 5, cap: 1110 },
    "B01": { slots: 37, tier: 5, cap: 1110 }, "B02": { slots: 37, tier: 5, cap: 1110 }, 
    "B03": { slots: 37, tier: 5, cap: 1110 }, "B04": { slots: 37, tier: 5, cap: 1110 },
    "B05": { slots: 37, tier: 5, cap: 1110 }, "B06": { slots: 37, tier: 5, cap: 1110 },
    "B07": { slots: 23, tier: 5, cap: 690 },  "B08": { slots: 23, tier: 5, cap: 690 },
    "C01": { slots: 45, tier: 3, cap: 810 },  "C02": { slots: 45, tier: 4, cap: 1080 }, 
    "C03": { slots: 45, tier: 5, cap: 1350 }, "C04": { slots: 45, tier: 5, cap: 1350 }, 
    "C05": { slots: 45, tier: 5, cap: 1350 }, "C06": { slots: 45, tier: 5, cap: 1350 }, 
    "C07": { slots: 45, tier: 5, cap: 1350 }, "C08": { slots: 45, tier: 5, cap: 1350 },
    "BR9": { slots: 18, tier: 5, cap: 540 },  "RC9": { slots: 12, tier: 5, cap: 360 }, 
    "OOG": { slots: 45, tier: 1, cap: 270 }
};
...
// Load Capacity from LocalStorage or Default
...
function setProgress(pct, msg) {
    ...
    if(circle) circle.style.strokeDashoffset = 314.159 - (pct/100)*314.159;
...
function updateCapacity(block, newSlots, newTier) {
    ...
    if (newSlots !== null) activeCapacity[block].slots = Number(newSlots);
    if (newTier !== null) activeCapacity[block].tier = Number(newTier);

    activeCapacity[block].cap = Math.round(activeCapacity[block].slots * activeCapacity[block].tier * 6);
...
document.getElementById('fileInv').addEventListener('change', function(e) {
    ...
    reader.onload = function(evt) {
        try {
            ...
            if(hIdx === -1 || colMap.carrier === -1) throw new Error("Format Kolom Excel tidak dikenali.");
            ...
            for(let i=hIdx+1; i<json.length; i++) {
                ...
                if(parsedBlock.length < 2) continue; 
...
function renderOverview() {
    if(!isInvLoaded) return;
    
    const tbody = document.getElementById('tableBody');
    tbody.innerHTML = '';
    
    let yardMap = {};
    // Ensure all blocks in activeCapacity are initialized
    Object.keys(activeCapacity).forEach(b => {
        yardMap[b] = { c20:0, c40:0, c45:0, impT:0, expT:0, impCount:0, expCount:0, empty:0 };
    });

    // Populate Data
    invData.forEach(it => {
        if(!yardMap[it.block]) yardMap[it.block] = { c20:0, c40:0, c45:0, impT:0, expT:0, impCount:0, expCount:0, empty:0 };
        
        let teus = 1;
        if(it.length.startsWith('20')) { yardMap[it.block].c20++; teus = 1; }
        else if(it.length.startsWith('45')) { yardMap[it.block].c45++; teus = 2.25; }
        else { yardMap[it.block].c40++; teus = 2; }

        if(it.move.includes('import') || it.move.includes('disc') || it.move.includes('vessel')) {
            yardMap[it.block].impT += teus; 
            yardMap[it.block].impCount++;
        } else {
            yardMap[it.block].expT += teus; 
            yardMap[it.block].expCount++;
        }

        if(it.loadStatus.includes('EMPTY') || it.loadStatus === 'MT') {
            yardMap[it.block].empty++;
        }
    });
    ...
    Object.keys(yardMap).sort().forEach(b => {
        // Safe check if block exists in capacity config, if not create dummy
        if(!activeCapacity[b]) activeCapacity[b] = { slots:0, tier:0, cap:0 };
        
        let d = yardMap[b];
        let cap = activeCapacity[b];
        ...
        if(!EXCLUDED_BLOCKS_YARD.includes(b)) {
            ...
            if(totT > 0) {
                s.impC += cap.cap * (d.impT / totT); 
                s.expC += cap.cap * (d.expT / totT);
            } else {
                if(EXPORT_DEFAULTS.includes(b)) s.expC += cap.cap; 
                else s.impC += cap.cap;
            }
            s.impS += d.impT; 
            s.expS += d.expT;
        ...
        if (b === "OOG") { remark = "OOG Area"; rCls = "row-oog"; }
        else if (b === "RC9" || b === "BR9") { remark = "Reefer Area"; rCls = "row-reefer"; }
        else if (b === "C01" || b === "C02") { remark = "DG Area"; rCls = "row-dg"; }
        else {
            if (occ < 3 && totBox < 5) { remark = "Zero Block"; rCls = "row-zero"; } 
            else if (totBox > 0) {
                let iPct = (d.impT / (d.impT + d.expT)) * 100;
                let ePct = 100 - iPct;
                if (iPct >= 95) { remark = "Import Only"; rCls = "row-import"; }
                else if (ePct >= 95) { remark = "Export Only"; rCls = "row-export"; }
                else { remark = `${Math.round(iPct)}% Imp | ${Math.round(ePct)}% Exp`; rCls = "row-mixed"; }
            }
        }

        if(d.empty > 0) remark += ` <span class='text-[9px] bg-blue-100 text-blue-700 px-1 rounded ml-1'>${d.empty} MT</span>`;
        ...
        tbody.innerHTML += `
            <tr class="${rCls} hover:bg-slate-50 transition-colors">
                <td class="p-2 border-r border-slate-200/50 font-bold text-center relative">
                    <div class="absolute inset-y-0 left-0 w-1 ${barColor} rounded-r"></div>${b}
                </td>
                <td class="p-2 border-r border-slate-200/50 text-center">
                    <div class="flex items-center gap-2">
                        <div class="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden shadow-inner">
                            <div class="${barColor} h-full" style="width:${Math.min(occ,100)}%"></div>
                        </div>
                        <span class="w-10 text-right font-mono occupancy-pct">${Math.round(occ)}%</span>
                    </div>
                </td>
                <td class="p-2 border-r border-slate-200/50 text-left font-bold text-[10px]">${remark}</td>
                ...
                <td class="col-detail p-1 text-center border-r border-slate-200/50">
                    <input type="number" min="1" class="w-16 h-8 text-sm text-center appearance-none bg-white/70 border border-slate-300 rounded" value="${cap.slots}" onchange="updateCapacity('${b}', this.value, null)">
                </td>
                <td class="col-detail p-1 text-center border-r border-slate-200/50">
                    <input type="number" min="1" step="0.5" class="w-16 h-8 text-sm text-center appearance-none bg-white/70 border border-slate-300 rounded" value="${cap.tier}" onchange="updateCapacity('${b}', null, this.value)">
                </td>
                <td class="p-1 text-center border-r border-slate-200/50 font-bold text-xs text-slate-500">${cap.cap}</td>
            </tr>`;
    });
    ...
    // Timestamp
    const now = new Date();
    document.getElementById('lastUpdated').innerText = `Generated: ${now.toLocaleDateString()}, ${now.toLocaleTimeString()}`;
}
...
function updateSummary(s) {
    ...
    document.getElementById('sumImpCap').innerText = Math.round(s.impC).toLocaleString();
    document.getElementById('sumExpCap').innerText = Math.round(s.expC).toLocaleString();
    document.getElementById('sumTotalCap').innerText = Math.round(totalC).toLocaleString();
...
// --- TAB 2: ANALYTICS (Carrier Spreading) ---
function renderClusterSpreading() {
    const body = document.getElementById('clusterBody');
    if(!body) return;
    
    const showAll = document.getElementById('toggleSmallCarriers') ? document.getElementById('toggleSmallCarriers').checked : false;
    const chartDiv = document.getElementById('carrierChart');
    body.innerHTML = '';
    
    let stats = {};
    invData.forEach(it => {
        // Filter: Only Export logic for spreading usually makes sense, but let's include all for now or filter
    ...
    if(!sorted.length) { body.innerHTML = '<tr><td colspan="3" class="p-8 text-center text-slate-400">No Export Data Available.</td></tr>'; return; }
...
async function processClashAnalysis() {
    ...
    reader.onload = function(e) {
        try {
            ...
            if(nIdx === -1 || etaIdx === -1) throw new Error("Schedule format not recognized. Need Vessel Name and ETA.");
            ...
            for(let i=1; i<json.length; i++){
                ...
                if(r[nIdx]) {
                    ...
                    if(dEta) {
                        if(!dEtd) dEtd = new Date(dEta.getTime() + 86400000); // Default 24h if no ETD
...
function runClashLogic(schedule, inventory) {
    ...
    // Check if element exists before rendering stats (Tab 3 might be hidden)
    if(document.getElementById('blockStatsBody')) renderBlockStats(blockStats);
}
...
function renderCurrentClashes() {
    ...
    const currentSort = document.getElementById('clashSort') ? document.getElementById('clashSort').value : "severity";
    ...
    filtered.forEach(c => {
        ...
        const w1 = Math.max(((c.v1.eS-c.v1.sS)/max)*100, 4), l1 = ((c.v1.sS)/max)*100;
        const w2 = Math.max(((c.v2.eS-c.v2.sS)/max)*100, 4), l2 = ((c.v2.sS)/max)*100;
        
        let oHTML = "", conflictText = "";
        
        if(c.slotOverlap && c.v1.eS > 0) {
            const oS = Math.max(c.v1.sS, c.v2.sS), oE = Math.min(c.v1.eS, c.v2.eS);
            const oW = ((oE - oS) / max) * 100, oL = (oS / max) * 100;
            oHTML = `<div class="absolute top-0 bottom-0 bg-red-500/50 border-x border-red-600 z-10 animate-pulse" style="left:${oL}%; width:${Math.max(oW, 2)}%"></div>`;
            conflictText = `<span class="bg-red-600 text-white px-2 py-0.5 rounded shadow-sm">CRITICAL OVERLAP</span>`;
        } else {
            conflictText = `<span class="bg-amber-100 text-amber-800 border border-amber-200 px-2 py-0.5 rounded">Gap: ${c.slotDist}</span>`;
        }
        ...
        feed.innerHTML += `
            <div class="glass-panel p-4 rounded-xl mb-3 border border-white/50 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                <div class="flex justify-between items-start mb-3 relative z-10">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-slate-800 text-white rounded-xl flex items-center justify-center font-black text-xl shadow-lg shadow-slate-200">${c.block}</div>
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                ${conflictText}
                                <span class="text-[10px] font-mono text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">${t1} vs ${t2}</span>
                            </div>
                            <div class="font-bold text-slate-700 text-sm">${c.v1.v} <span class="text-slate-400 px-1">vs</span> ${c.v2.v}</div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-2xl font-black text-slate-800">${c.total}</div>
                        <div class="text-[9px] uppercase font-bold text-slate-400">Total Units</div>
                    </div>
                </div>
                
                <!-- Visualization Track -->
                <div class="relative h-6 bg-slate-100 rounded-md overflow-hidden border border-slate-200/60">
                    <!-- Vessel 1 -->
                    <div class="absolute top-0 bottom-0 bg-blue-500 hover:bg-blue-600 transition-colors opacity-90" style="left:${l1}%; width:${w1}%" title="${c.v1.v}"></div>
                    <!-- Vessel 2 -->
                    <div class="absolute top-0 bottom-0 bg-amber-400 hover:bg-amber-500 transition-colors opacity-90" style="left:${l2}%; width:${w2}%" title="${c.v2.v}"></div>
                    <!-- Overlap Highlight -->
                    ${oHTML}
                </div>
                <div class="flex justify-between text-[9px] text-slate-400 font-mono mt-1 px-1">
                    <span>Slot 01</span>
                    <span>Slot ${max}</span>
                </div>
            </div>`;
...
function renderBlockStats(stats) {
    ...
    sorted.forEach(([b, d]) => {
        ...
        body.innerHTML += `
            <tr class="stats-row cursor-pointer hover:bg-blue-50 transition-colors border-b border-slate-100 last:border-0" onclick="filterByBlock('${b}')" data-block="${b}">
                <td class="p-3 font-bold text-slate-700">${b}</td>
                <td class="p-3 text-[10px] text-slate-500 truncate max-w-[150px]" title="${vesselStr}">${vesselStr}</td>
                <td class="p-3 text-center"><span class="bg-red-100 text-red-600 px-2 py-0.5 rounded-full text-[10px] font-bold">${d.c}</span></td>
                <td class="p-3 text-right font-mono font-bold text-slate-700">${d.v}</td>
            </tr>`;
    });
}
...
function filterByBlock(block) {
    ...
    // UI updates for filter would go here if elements exist
    renderCurrentClashes();
}
...
function renderEmptySummary() {
    ...
    if(!impDiv || !expBody) return;

    let emptyData = invData.filter(d => d.loadStatus.includes('EMPTY') || d.loadStatus === 'MT' || d.loadStatus === 'E');
    ...
    impDiv.innerHTML = `
        ...
        <div class="bg-emerald-50 p-4 rounded-xl text-center border border-emerald-100 ring-2 ring-emerald-100 shadow-sm">
    ...
    if(sortedExp.length === 0) {
        expBody.innerHTML = '<tr><td colspan="5" class="p-8 text-center text-slate-400 italic">No Export Empty found.</td></tr>';
    } else {
        sortedExp.forEach(([carrier, s]) => {
            expBody.innerHTML += `
                <tr class="hover:bg-slate-50 transition-colors border-b border-slate-50">
                    <td class="px-6 py-3 font-bold text-slate-700">${carrier}</td>
                    <td class="px-6 py-3 text-center text-slate-600">${s.c20 || '-'}</td>
                    <td class="px-6 py-3 text-center text-slate-600">${s.c40 || '-'}</td>
                    <td class="px-6 py-3 text-center text-slate-600">${s.c45 || '-'}</td>
                    <td class="px-6 py-3 text-center font-bold bg-slate-50 text-slate-800 border-l border-slate-100">${s.total}</td>
                </tr>`;
        });
        ...
        expBody.innerHTML += `
            <tr class="bg-slate-100 border-t-2 border-slate-200 font-bold text-sm">
                <td class="px-6 py-3 text-slate-800 uppercase tracking-wider">Grand Total</td>
                ...
                <td class="px-6 py-3 text-center text-emerald-600 text-lg border-l border-slate-200 bg-emerald-50">${grand.total}</td>
            </tr>`;
    }
}
...
function switchTab(t) {
    ['overview', 'analytics', 'clash', 'empty'].forEach(id => {
        ...
        if(btn) btn.classList.replace('active', 'inactive');
    });
    ...
    if(targetBtn) targetBtn.classList.replace('inactive', 'active');
}

function toggleDetails() { 
    const tbl = document.getElementById('mainTable');
    if(tbl) tbl.classList.toggle('show-details'); 
}

function clearCache() { 
    if(confirm("Clear loaded data?")){ location.reload(); } 
}
...
function downloadImage() {
    ...
    if (!document.getElementById('tab-analytics').classList.contains('hidden')) {
        activeId = 'captureAreaAnalytics'; fileName = 'Analytics';
    } else if (!document.getElementById('tab-clash').classList.contains('hidden')) {
        activeId = 'captureAreaClash'; fileName = 'Clash';
    } else if (document.getElementById('tab-empty') && !document.getElementById('tab-empty').classList.contains('hidden')) {
        activeId = 'captureAreaEmpty'; fileName = 'Empty_Summary';
    }

    const element = document.getElementById(activeId);
    if(!element) { alert("Nothing to capture!"); return; }

    // Use font loading check to ensure icons render
...
function exportToExcel() {
    ...
    if(mainTable) {
        const ws1 = XLSX.utils.table_to_sheet(mainTable);
        XLSX.utils.book_append_sheet(wb, ws1, "Yard Overview");
    }
    ...
    if(emptyData.length > 0) {
        ...
        XLSX.utils.book_append_sheet(wb, wsEmpty, "Empty Containers");
    }

    // Sheet 3: Full Data Dump
    if(invData.length > 0) {
         const wsAll = XLSX.utils.json_to_sheet(invData);
         XLSX.utils.book_append_sheet(wb, wsAll, "Parsed Data");
    }

    XLSX.writeFile(wb, `NPCT1_Yard_Data_${new Date().toISOString().slice(0,10)}.xlsx`);
}
